...
- ベクトルデータベースに格納され検索されるデータは、使用される埋め込みモデルの最大シーケンス長（BERTのようなモデルの場合、これは数百語以下）に収める必要がある。現在のところ、これを実現する最善の方法は、LangChainやLlamaIndex[^8]のようなフレームワークを利用して、モデルのコンテキストに合うように固定サイズのベクトルにデータをチャンク分割するか、圧縮することである。
